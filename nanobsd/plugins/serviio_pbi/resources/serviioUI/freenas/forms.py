...
class ServiioForm(forms.ModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        for field in obj._meta.local_fields:
            if field.attname not in utils.serviio_advanced_vars:
                continue
            info = utils.serviio_advanced_vars.get(field.attname)
        ...
        #if obj.download_dir:
        #    #os.system("/usr/sbin/chown -R dlna:dlna '%s'" % main_settings["serviio_download_dir"])
        ...
        with open(utils.serviio_config, "w") as f:
            f.write("[general]\n")
            f.write("web_root = /usr/pbi/serviio-%s/share/mt-daapd/admin-root\n" % (
                platform.machine(),
                ))
            f.write("port = %d\n" % (obj.port, ))
            f.write("admin_pw = %s\n" % (obj.admin_pw, ))
