...
from serviioUI.freenas import forms, models, utils
...
def start(request):
    serviio_key, serviio_secret = utils.get_serviio_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=serviio_key,
        secret=serviio_secret)
    ...
    try:
        serviio = models.Serviio.objects.order_by('-id')[0]
        serviio.enable = True
        serviio.save()
    except IndexError:
...
def stop(request):
    ...
    try:
        serviio = models.Serviio.objects.order_by('-id')[0]
        serviio.enable = False
        serviio.save()
    except IndexError:
        serviio = models.Serviio.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.serviio_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def edit(request):
    ...
    """
    Get the Serviio object
    If it does not exist create a new entry
    """
    try:
        serviio = models.Serviio.objects.order_by('-id')[0]
    except IndexError:
        serviio = models.Serviio.objects.create()
    ...
    if request.method == "GET":
        form = forms.ServiioForm(instance=serviio,
            jail=jail)
    ...
    form = forms.ServiioForm(request.POST,
        instance=serviio,
        jail=jail)
    if form.is_valid():
        form.save()
        return JsonResponse(request, error=True, message="Serviio settings successfully saved.")
...
def treemenu(request):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('serviio_edit'),
        'kwargs': {'plugin_name': 'serviio'},
    }
...
def treemenu_icon(request):
    with open(utils.serviio_icon, 'rb') as f:
        icon = f.read()
